...
public class OVRSkeleton : MonoBehaviour
{
    ...
    public bool FreezeIndex { get; set; }
    public bool FreezeMiddle { get; set; }
    public bool FreezeRing { get; set; }
    public bool FreezePinky { get; set; }
	...
	private void Update()
	{
		...
		if (data.IsDataValid)
		{
            ...
            //Determine if is colliding
            List<bool> isCollision = new List<bool>();
            for (var i = 0; i < _capsules.Count; ++i)
            {
                Vector3 capsuleStart = _capsules[i].CapsuleCollider.center - _capsules[i].CapsuleCollider.height / 2.0f * (_capsules[i].CapsuleCollider.transform.localEulerAngles);
                Vector3 capsuleEnd = _capsules[i].CapsuleCollider.center + _capsules[i].CapsuleCollider.height / 2.0f * (_capsules[i].CapsuleCollider.transform.localEulerAngles);
                isCollision.Add(Physics.CheckCapsule(capsuleStart, capsuleEnd, _capsules[i].CapsuleCollider.radius, -1, QueryTriggerInteraction.Collide));
                
            }

            for (var i = 0; i < _bones.Count; ++i) //TODO Set if not touching
			{
				if (_bones[i].Transform != null)
				{
                    //if(isCollision[i])
                    if (!(FreezeIndex  && i == 7 || i == 8 || i == 9)
                        || !(FreezeMiddle && i == 10 || i == 11 || i == 12)
                        || !(FreezeRing && i == 13 || i == 14 || i == 15))
                        {
...
public class OVRBoneCapsule : MonoBehaviour
{
